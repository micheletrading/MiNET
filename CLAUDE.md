...
- Machine-readable protocol references: Mojang's official docs repo (github.com/Mojang/bedrock-protocol-docs, JSON Schemas for the current protocol only) and PrismarineJS/minecraft-data (per-version ProtoDef definitions in `data/bedrock/<version>/protocol.json`, plus block/item data dumps). For historical formats use minecraft-data; for the newest protocol use Mojang's schemas.
